...
import type { KRAAssignment, KRAEvaluation } from '@/hooks/useKRA';
...
export function KRAEmployeeForm({ assignment, isReadOnly = false, onClose }: KRAEmployeeFormProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Goals */}
      <div className="space-y-6">
        {detailedAssignment.template?.goals?.map((goal, index) => {
          const evaluation = evaluations[goal.id] || {};
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-2 gap-2">
                    {[1, 2, 3, 4, 5].map(level => {
                      ...
                      return (
                        ...
                        >
                          <div className="font-medium text-start">Level {level}</div>
                          <div className="text-muted-foreground text-start">{rating}</div>
                          <div className="mt-1">
