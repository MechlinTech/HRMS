...
export function AssetManagement() {
  ...
  const { data: managerAssignments } = useManagerAssetAssignments(); 
  ...
  const assignmentsData = isHRAdmin ? assignments : managerAssignments;
  ...
  try {
    ...
    // Post-filter by employee status when specified
    if (assignmentFilters.employee_status && assignmentFilters.employee_status !== 'all') {
      const desired = assignmentFilters.employee_status;
      filteredAssignments = filteredAssignments.filter((assignment: any) => {
        const status = resolveEmployeeStatus(assignment);
        return desired === 'active' ? status === 'active' : status === 'inactive';
      });
    }
    // Exclude archived assets from All Assets tab display
    const nonArchivedAssets = assets ? assets.filter(asset => asset.status !== 'archived') : [];
    filteredAssets = nonArchivedAssets ? applyFilters(nonArchivedAssets, assetFilters) : [];
  ...
  useEffect(() => {
    ...
    if (vmData && isEditVMDialogOpen) {
      ...
      editVMForm.reset({
        ...
        current_password: vmData.current_password_hash || '',
        previous_password: vmData.previous_password_hash || '',
        ...
        approval_date: vmData.approval_date ? new Date(vmData.approval_date) : undefined,
        expiry_date: vmData.expiry_date ? new Date(vmData.expiry_date) : undefined,
      });
    }
  ...
  const handleEditAsset = (asset: any) => {
    ...
    assetForm.reset({
      ...
      notes: asset.notes || '',
      // New extended fields
      insurance_warranty_extended: asset.insurance_warranty_extended ? new Date(asset.insurance_warranty_extended) : undefined,
      previous_audit_date: asset.previous_audit_date ? new Date(asset.previous_audit_date) : undefined,
      hardware_image_date: asset.hardware_image_date ? new Date(asset.hardware_image_date) : undefined,
  ...
  const handleEditAssignment = (assignment: any) => {
    ...
    editAssignmentForm.reset({
      ...
      assignment_expiry_date: assignment.assignment_expiry_date ? new Date(assignment.assignment_expiry_date) : undefined,
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <CardDescription className="text-sm">
                              Created by {guidance.created_by_user?.full_name || 'Unknown'} on{' '}
                              {new Date(guidance.created_at).toLocaleDateString()}
                              {guidance.updated_at !== guidance.created_at && (
                                <span className="text-muted-foreground">
                                  {' '}• Updated on {new Date(guidance.updated_at).toLocaleDateString()}
                                </span>
                              )}
                            </CardDescription>
                          </div>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                                  ...
                                  <div className="grid grid-cols-2 gap-4 text-sm">
                                    ...
                                    {selectedAssignment.assignment_expiry_date && (
                                      <div>
                                        ...
                                        <p className="text-muted-foreground">
                                          {format(new Date(selectedAssignment.assignment_expiry_date), 'MMM dd, yyyy')}
                                        </p>
                                      </div>
                                    )}
                                  </div>
                        ...
                        <Button 
                          ...
                          variant="outline"
                          onClick={() => {
                            // All assignments use the same edit modal
                            setSelectedAssignment(assignment);
                            setIsEditAssignmentDialogOpen(true);
                          }}
                        >
                                  ...
                                  return (
                                    <div key={log.id} className="relative">
                                      <div className={`flex items-start gap-4 p-3 rounded-lg border ${
                                        isEmployeeInactive && isActive ? 'bg-red-50 border-red-200' : 'bg-white'
                                      }`}>
                                        {/* Timeline dot */}
                                        ...
                                        <div className="flex-1 min-w-0">
                                          <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                              <IconComponent className="h-5 w-5 text-muted-foreground" />
                                              <div>
                                                <div className="font-medium">{log.asset_name}</div>
                                                <div className="text-sm text-muted-foreground">
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm bg-muted px-2 py-1 rounded">{currentVM.current_password_hash || 'Not configured'}</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div>
                      <p className="font-medium text-sm text-gray-600">Previous Password</p>
                      <p className="text-sm bg-muted px-2 py-1 rounded">{currentVM.previous_password_hash || 'Not configured'}</p>
                    </div>
                  </div>
                </div>
              </div>
