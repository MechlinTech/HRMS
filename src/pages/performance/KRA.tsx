...
import { useKRAPermissions, useCanAccessKRADashboard } from '@/hooks/useKRAPermissions';

export function KRA() {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState('dashboard');
  const permissions = useKRAPermissions();
  const canAccess = useCanAccessKRADashboard();
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className={`grid w-full max-w-md ${
          permissions.canCreateTemplates && permissions.canViewAssignments ? 'grid-cols-3' : 
          permissions.canViewAssignments ? 'grid-cols-2' : 'grid-cols-1'
        }`}>
